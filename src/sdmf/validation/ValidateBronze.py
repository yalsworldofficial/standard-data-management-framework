...
class ValidateBronze(ValidateDataSpecsBase):
    ...
    def check_data_types(self):
        ...
        for col, expected in expected_types.items():
            ...
            if expected == "int":
                if not series.map(lambda x: isinstance(x, int) or x is None).all():
                    bad_types.append((col, "int"))

            if expected == "str":
                if not series.map(lambda x: isinstance(x, str) or x is None).all():

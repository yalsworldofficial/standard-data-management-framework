...
class DataLoadController():
    ...
    def __init__(self, spark: SparkSession, allowed_df: pd.DataFrame, config: configparser.ConfigParser) -> None:
    ...
    def __run_group(self, group) -> list[LoadResult]:
        ...
        self.logger.info(
            f"Starting group {group_id} with {len(feeds)} feed(s)"
        )
    ...
    def __prepare(self):
        ...
        grouped = (
            self.master_specs_df
            .sort_values("parallelism_group_number")
            .groupby("parallelism_group_number", sort=False)
        )
        ...
        for key, group in grouped:
            self.logger.info(
                f"Execution group {key}, Total Feeds: {len(group)}"
            )
